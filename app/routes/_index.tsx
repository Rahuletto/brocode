...
export default function IndexPage() {
  ...
  type EncodeResponse =
    | { audioData: string; fileName: string }
    | { error: string };
  ...
  useEffect(() => {
    ...
    return () => {
      if (audioContextRef.current && audioContextRef.current.state !== "closed")
        audioContextRef.current
          .close()
          .catch((e) => console.error("Error closing AC:", e));
    };
  }, []);
  ...
  useEffect(() => {
    ...
    if (encodeFetcher.state === "loading") {
      setIsEncoding(true);
    ...
    if (encodeFetcher.data && encodeFetcher.state === "idle") {
      ...
      if (
        typeof encodeFetcher.data === "object" &&
        encodeFetcher.data !== null
      ) {
        if (
          "audioData" in encodeFetcher.data &&
          typeof encodeFetcher.data.audioData === "string"
        ) {
          const blob = base64ToBlob(encodeFetcher.data.audioData);
          if (blob) {
            try {
              currentAudioSrc = URL.createObjectURL(blob);
              setAudioSrc(currentAudioSrc);
            } catch (e) {
              newErrorMessage = "Failed url.";
              console.error(e);
            }
          } else {
            newErrorMessage = "Failed blob.";
          }
        } else if ("error" in encodeFetcher.data) {
          newErrorMessage = `Encoding Error: ${encodeFetcher.data.error}`;
          setAudioSrc(null);
        } else {
          newErrorMessage = "Unexpected JSON.";
          console.error("Unexpected JSON:", encodeFetcher.data);
          setAudioSrc(null);
        }
      } else {
        newErrorMessage = "Invalid response.";
        console.error("Non-object response:", encodeFetcher.data);
        setAudioSrc(null);
      }
  ...
  useEffect(() => {
    if (decodeFetcher.state === "loading") {
      setIsDecoding(true);
      setErrorMessage(null);
    } else {
      setIsDecoding(false);
    ...
    if (decodeFetcher.data && decodeFetcher.state === "idle") {
      if (
        typeof decodeFetcher.data === "object" &&
        decodeFetcher.data !== null
      ) {
        if (
          "decodedText" in decodeFetcher.data &&
          typeof decodeFetcher.data.decodedText === "string"
        ) {
          setDecodedMessage(decodeFetcher.data.decodedText);
          setErrorMessage(null);
          console.log("Decode successful (server).");
        } else if ("error" in decodeFetcher.data) {
          setErrorMessage(`Decoding Error: ${decodeFetcher.data.error}`);
          setDecodedMessage("");
          console.error(`Server decode err: ${decodeFetcher.data.error}`);
        } else {
          setErrorMessage("Unexpected decode response.");
          setDecodedMessage("");
          console.error("Unexpected decode JSON:", decodeFetcher.data);
        }
      } else {
        setErrorMessage("Invalid decode response.");
        setDecodedMessage("");
        console.error("Non-object decode response:", decodeFetcher.data);
      }
    }
  }, [decodeFetcher.state, decodeFetcher.data]);

  const handleEncode = useCallback(() => {
    if (isEncoding || !messageToEncode.trim()) return;
    const formData = new FormData();
    formData.append("message", messageToEncode);
    encodeFetcher.submit(formData, { method: "post", action: "/api/encode" });
  }, [messageToEncode, encodeFetcher, isEncoding]);
  ...
  const handleDecode = useCallback(() => {
    if (!uploadedFile) {
      setErrorMessage("No audio file selected.");
      return;
    }
    if (isDecoding) return;
    console.log(`Sending file "${uploadedFile.name}" to decode API...`);
    setErrorMessage(null);
    setDecodedMessage("");
    const formData = new FormData();
    formData.append("audioFile", uploadedFile);
    decodeFetcher.submit(formData, {
      method: "post",
      action: "/api/decode",
      encType: "multipart/form-data",
    });
  }, [uploadedFile, decodeFetcher, isDecoding]);
    ...
    <main className="bg-background min-h-screen px-4 sm:px-12 md:px-24 py-10 md:py-14 text-foreground">
      ...
      {errorMessage && (
        <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 w-full max-w-md px-4">
          <div
            className="text-red-100 p-3 px-5 rounded-lg bg-red-600 border border-red-700 shadow-lg"
            role="alert"
          >
      ...
      <div className="max-w-3xl mx-auto h-full mt-12 min-h-[70vh] flex items-center flex-col justify-center">
          ...
          <section className="p-4 md:p-6 w-full">
            ...
            <div>
              ...
              <textarea
                id="message-input"
                rows={5}
                className="w-full px-4 py-3 bg-foreground/5 rounded-xl mt-1 font-regular focus:ring-2 focus:ring-foreground/50 focus:border-foreground/50 transition"
            ...
            >
              {isEncoding ? (
                <>
                  <svg
                    className="animate-spin -ml-1 mr-2 h-5 w-5 text-background"
        ...
        {activeTab === "decode" && (
          <section className="space-y-6 p-4 md:p-6 w-full">
            ...
            >
              {isDecoding ? (
                <>
                  <RiLoader2Fill className="text-background animate-spin" />
                  Decoding...
                </>
              ) : (
                "Decode Audio"
            ...
            <div className="mt-6">
              ...
              <textarea
                id="decoded-output"
                rows={5}
                className="w-full px-4 py-3 bg-foreground/5 rounded-xl mt-1 font-regular opacity-80 cursor-default"
            ...
            <div className="flex items-center gap-4">
              ...
              {audioSrc.startsWith("blob:") && (
                <a
                  href={audioSrc}
                  download={
                    encodeFetcher.data && "fileName" in encodeFetcher.data
                      ? encodeFetcher.data.fileName
                      : "encoded_audio.wav"
                  }
                  className="ml-64 text-sm text-foreground/70 hover:text-foreground underline"
                >
                  Download
                </a>
              )}
